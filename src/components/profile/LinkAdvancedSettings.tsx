...
const FEATURED_STYLES = [
  ...
  { value: 'outline', label: 'Outline', preview: 'border-2 border-dashed' },
  {
    value: 'neon',
    label: 'Neon',
    preview: 'ring-2 ring-primary ring-offset-2',
  },
]

export function LinkAdvancedSettings({
  link,
  isCreator,
  onClose,
}: LinkAdvancedSettingsProps) {
  const { theme } = useTheme()
  const queryClient = useQueryClient()
  const [activeTab, setActiveTab] = useState<
    'analytics' | 'featured' | 'schedule'
  >('analytics')
  ...
  const featuredMutation = useMutation({
    mutationFn: () =>
      updateFeatured({
        data: {
          linkId: link.id,
          isFeatured: localFeatured.isFeatured,
          featuredStyle: localFeatured.featuredStyle as
            | 'default'
            | 'glow'
            | 'gradient'
            | 'outline'
  ...
  const scheduleMutation = useMutation({
    mutationFn: () =>
      updateSchedule({ data: { linkId: link.id, schedule: localSchedule } }),
    onSuccess: () =>
      void queryClient.invalidateQueries({ queryKey: ['my-links'] }),
  })
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="p-3 border-b flex gap-1 overflow-x-auto"
          style={{ borderColor: theme.colors.border }}
        >
          {[
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {Object.entries(analyticsData.deviceStats || {}).map(
                          ([device, count]) => (
                            <span
                              key={device}
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {Object.entries(analyticsData.browserStats || {}).map(
                          ([browser, count]) => (
                            <span
                              key={browser}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-12 gap-1">
                        {Array.from({ length: 24 }, (_, hour) => {
                          const totalForHour = Object.entries(
                            analyticsData.hourlyClicks || {},
                          )
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p
                      ...
                      style={{ color: theme.colors.foregroundMuted }}
                    >
                      Highlight this link with special styling
                    </p>
                  </div>
                  <div
                    className="relative w-12 h-6 rounded-full transition-all cursor-pointer"
                    style={{
                      background: localFeatured.isFeatured
                        ? '#ec4899'
                  ...
                  >
                    <div
                      ...
                      style={{
                        left: localFeatured.isFeatured ? '28px' : '4px',
                      }}
                    />
                  </div>
                </div>
                    ...
                    {FEATURED_STYLES.map((style) => (
                      <button
                        ...
                        onClick={() =>
                          setLocalFeatured({
                            ...localFeatured,
                            featuredStyle: style.value,
                          })
                        }
                        className="p-3 rounded-xl text-center transition-all"
                        style={{
                          background:
                            localFeatured.featuredStyle === style.value
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p
                      ...
                      style={{ color: theme.colors.foregroundMuted }}
                    >
                      Show/hide link at specific times
                    </p>
                  </div>
                  <div
                    className="relative w-12 h-6 rounded-full transition-all cursor-pointer"
                  ...
                  >
                    <div
                      className="absolute top-1 w-4 h-4 rounded-full bg-white transition-all"
                      style={{ left: localSchedule.enabled ? '28px' : '4px' }}
                    />
                  </div>
                </div>
