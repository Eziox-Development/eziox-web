...
export function BadgesTab({
  ...
  const allBadges = BADGE_LIST.filter(
    (b) => !b.adminOnly || safeBadges.includes(b.id),
  )
      ...
      >
        ...
        <div className="p-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {allBadges
              .filter((b) => b.adminOnly && !['owner', 'admin'].includes(b.id))
              .map((badge, index) => (
                <BadgeCard
                  key={badge.id}
                  badge={badge}
  ...
  return (
    ...
    >
        ...
        {(() => {
          ...
          return IconComponent ? (
            <IconComponent
              size={32}
              style={{
                color: earned ? badge.color : 'var(--foreground-muted)',
              }}
              className="mx-auto"
            />
          ) : null
      ...
      <p
        ...
        style={{ color: earned ? badge.color : 'var(--foreground-muted)' }}
      >
        {badge.name}
      </p>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span
              ...
              style={{ color: earned ? badge.color : 'var(--foreground)' }}
            >
              {badge.name}
            </span>
            {earned && <CheckCircle size={14} className="text-green-500" />}
            {locked && (
              <Lock size={14} style={{ color: 'var(--foreground-muted)' }} />
            )}
          </div>
          <p className="text-sm" style={{ color: 'var(--foreground-muted)' }}>
            {description}
