...
import { publishTemplateFn, getMyTemplatesFn, deleteTemplateFn } from '@/server/functions/templates'
import type { AnimatedProfileSettings, OpenGraphSettings } from '@/server/db/schema'
...
const PRESET_FONTS = [
  ...
  { name: 'Roboto', url: 'https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap', category: 'sans-serif' },
  { name: 'Open Sans', url: 'https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;500;600;700&display=swap', category: 'sans-serif' },
  { name: 'Lato', url: 'https://fonts.googleapis.com/css2?family=Lato:wght@400;700&display=swap', category: 'sans-serif' },
  { name: 'Nunito', url: 'https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700&display=swap', category: 'sans-serif' },
  { name: 'Raleway', url: 'https://fonts.googleapis.com/css2?family=Raleway:wght@400;500;600;700&display=swap', category: 'sans-serif' },
  { name: 'Playfair Display', url: 'https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;600;700&display=swap', category: 'serif' },
  { name: 'Merriweather', url: 'https://fonts.googleapis.com/css2?family=Merriweather:wght@400;700&display=swap', category: 'serif' },
  { name: 'Lora', url: 'https://fonts.googleapis.com/css2?family=Lora:wght@400;500;600;700&display=swap', category: 'serif' },
  { name: 'Source Serif Pro', url: 'https://fonts.googleapis.com/css2?family=Source+Serif+Pro:wght@400;600;700&display=swap', category: 'serif' },
  ...
  { name: 'Fira Code', url: 'https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;600;700&display=swap', category: 'mono' },
  { name: 'Space Mono', url: 'https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&display=swap', category: 'mono' },
  { name: 'Bebas Neue', url: 'https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap', category: 'display' },
  { name: 'Righteous', url: 'https://fonts.googleapis.com/css2?family=Righteous&display=swap', category: 'display' },
  { name: 'Pacifico', url: 'https://fonts.googleapis.com/css2?family=Pacifico&display=swap', category: 'display' },
  { name: 'Permanent Marker', url: 'https://fonts.googleapis.com/css2?family=Permanent+Marker&display=swap', category: 'display' },
...
export function CreatorTab() {
  ...
  const [activeSection, setActiveSection] = useState<'css' | 'fonts' | 'animations' | 'opengraph' | 'publish'>('css')
  const [showPresetFonts, setShowPresetFonts] = useState(true)
  const [previewFont, setPreviewFont] = useState<string | null>(null)
  const [publishName, setPublishName] = useState('')
  const [publishDesc, setPublishDesc] = useState('')
  const [publishCategory, setPublishCategory] = useState<typeof TEMPLATE_CATEGORIES[number]>('creative')
  ...
  const publishMutation = useMutation({
    mutationFn: (data: { name: string; description?: string; category: typeof TEMPLATE_CATEGORIES[number]; isPublic: boolean }) => 
      publishTemplate({ data }),
    onSuccess: () => {
      toast.success('Template published!', { description: 'Your style is now available in the community.' })
      setPublishName('')
      setPublishDesc('')
    ...
    return (
      ...
      >
        ...
        <p className="text-sm mb-6 max-w-md mx-auto" style={{ color: theme.colors.foregroundMuted }}>
          Custom CSS, fonts, animations, and advanced controls are available with Creator tier.
        </p>
        <Link
          to="/profile"
  ...
  const currentAnimated = localAnimated.enabled ? localAnimated : (settings?.animatedProfile || DEFAULT_ANIMATED_SETTINGS)
  const currentOG = localOG.useCustom ? localOG : (settings?.openGraphSettings || DEFAULT_OG_SETTINGS)
      ...
      >
        {[
          ...
          <motion.button
            ...
            style={{
              background: activeSection === id ? 'linear-gradient(135deg, #ec4899, #8b5cf6)' : 'transparent',
      ...
      <AnimatePresence mode="wait">
          ...
          >
            <div className="p-5 border-b" style={{ borderColor: theme.colors.border }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: theme.colors.foregroundMuted }}>Add custom styles to your profile (sandboxed)</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 space-y-4">
              <div className="relative">
                ...
                />
                <div className="absolute bottom-3 right-3 text-xs" style={{ color: theme.colors.foregroundMuted }}>
              ...
              {cssErrors.length > 0 && (
                <div className="p-3 rounded-xl" style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium" style={{ color: '#ef4444' }}>CSS Validation Warnings</span>
                  </div>
              ...
              <div className="flex gap-3">
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                >
                  {cssMutation.isPending ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
                  Save CSS
                </motion.button>
                <motion.button
                  onClick={() => { setCssInput(''); cssMutation.mutate('') }}
          ...
          >
            <div className="p-5 border-b" style={{ borderColor: theme.colors.border }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold" style={{ color: theme.colors.foreground }}>Custom Fonts</h3>
                  <p className="text-xs" style={{ color: theme.colors.foregroundMuted }}>Add Google Fonts or custom font URLs</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 space-y-4">
              <div className="flex gap-2 mb-4">
                <button
                  ...
                  style={{
                    background: showPresetFonts ? theme.colors.primary : theme.colors.backgroundSecondary,
                    color: showPresetFonts ? '#fff' : theme.colors.foregroundMuted,
                  }}
                >
                  Preset Fonts
                </button>
              ...
              {showPresetFonts ? (
                <div className="space-y-3">
                  {previewFont && (
                    <link rel="stylesheet" href={PRESET_FONTS.find(f => f.name === previewFont)?.url} />
                  )}
                  {['sans-serif', 'serif', 'mono', 'display'].map((category) => (
                    <div key={category}>
                      <p className="text-xs font-medium uppercase tracking-wider mb-2" style={{ color: theme.colors.foregroundMuted }}>
                        {category === 'sans-serif' ? 'Sans Serif' : category === 'mono' ? 'Monospace' : category.charAt(0).toUpperCase() + category.slice(1)}
                      </p>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {PRESET_FONTS.filter(f => f.category === category).map((font) => {
                          const isAdded = settings?.customFonts?.some(cf => cf.name === font.name)
                          return (
                            <button
                              key={font.name}
                              onMouseEnter={() => setPreviewFont(font.name)}
                              onMouseLeave={() => setPreviewFont(null)}
                              onClick={() => !isAdded && fontMutation.mutate({ name: font.name, url: font.url, type: category === 'display' ? 'display' : 'body' })}
                              disabled={isAdded || fontMutation.isPending || (settings?.customFonts?.length || 0) >= 4}
                              className="p-3 rounded-xl text-left transition-all disabled:opacity-50"
                              style={{
                                background: isAdded ? `${theme.colors.primary}20` : theme.colors.backgroundSecondary,
                                border: `1px solid ${isAdded ? theme.colors.primary : theme.colors.border}`,
                                fontFamily: previewFont === font.name ? `'${font.name}', ${category}` : 'inherit',
                              }}
                            >
                              <p className="text-sm font-medium truncate" style={{ color: theme.colors.foreground }}>{font.name}</p>
                              {isAdded && <Check size={12} className="mt-1" style={{ color: theme.colors.primary }} />}
                            </button>
                          )
                        })}
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="flex gap-2">
                    <select
                      value={newFontType}
                      onChange={(e) => setNewFontType(e.target.value as 'display' | 'body')}
                    ...
                    <motion.button
                      onClick={() => newFontName && newFontUrl && fontMutation.mutate({ name: newFontName, url: newFontUrl, type: newFontType })}
                      ...
                      whileHover={{ scale: newFontName && newFontUrl ? 1.02 : 1 }}
                      whileTap={{ scale: 0.98 }}
                    >
                      {fontMutation.isPending ? <Loader2 size={16} className="animate-spin" /> : <Plus size={16} />}
                    </motion.button>
                  </div>
                </div>
              )}
              ...
              {settings?.customFonts && settings.customFonts.length > 0 && (
                <div className="pt-4 border-t" style={{ borderColor: theme.colors.border }}>
                  <p className="text-xs font-medium mb-3" style={{ color: theme.colors.foregroundMuted }}>Your Fonts ({settings.customFonts.length}/4)</p>
                  <div className="space-y-2">
                    {settings.customFonts.map((font) => (
                      ...
                      >
                        <div>
                          <p className="font-medium text-sm" style={{ color: theme.colors.foreground }}>{font.name}</p>
                          <p className="text-xs" style={{ color: theme.colors.foregroundMuted }}>{font.type} font</p>
                        </div>
                        <motion.button
          ...
          >
            <div className="p-5 border-b" style={{ borderColor: theme.colors.border }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold" style={{ color: theme.colors.foreground }}>Animated Profile</h3>
                    <p className="text-xs" style={{ color: theme.colors.foregroundMuted }}>Add micro-interactions and effects</p>
                  </div>
                </div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <span className="text-sm" style={{ color: theme.colors.foregroundMuted }}>Enable</span>
                  <div
                    className="relative w-12 h-6 rounded-full transition-all cursor-pointer"
                    style={{ background: currentAnimated.enabled ? '#22c55e' : theme.colors.backgroundSecondary }}
                    onClick={() => setLocalAnimated({ ...currentAnimated, enabled: !currentAnimated.enabled })}
                  >
                    <div
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setLocalAnimated({ ...currentAnimated, avatarAnimation: e.target.value as AnimatedProfileSettings['avatarAnimation'] })}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setLocalAnimated({ ...currentAnimated, bannerAnimation: e.target.value as AnimatedProfileSettings['bannerAnimation'] })}
                    className="w-full px-4 py-2.5 rounded-xl text-sm"
                    style={{ background: theme.colors.backgroundSecondary, color: theme.colors.foreground, border: `1px solid ${theme.colors.border}` }}
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setLocalAnimated({ ...currentAnimated, linkHoverEffect: e.target.value as AnimatedProfileSettings['linkHoverEffect'] })}
                    className="w-full px-4 py-2.5 rounded-xl text-sm"
                    style={{ background: theme.colors.backgroundSecondary, color: theme.colors.foreground, border: `1px solid ${theme.colors.border}` }}
                  >
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block" style={{ color: theme.colors.foreground }}>Page Transition</label>
                  <select
                    value={currentAnimated.pageTransition}
                    onChange={(e) => setLocalAnimated({ ...currentAnimated, pageTransition: e.target.value as AnimatedProfileSettings['pageTransition'] })}
                    className="w-full px-4 py-2.5 rounded-xl text-sm"
                    style={{ background: theme.colors.backgroundSecondary, color: theme.colors.foreground, border: `1px solid ${theme.colors.border}` }}
                  >
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
              >
                {animatedMutation.isPending ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
        ...
        {activeSection === 'opengraph' && (
          ...
          >
            <div className="p-5 border-b" style={{ borderColor: theme.colors.border }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold" style={{ color: theme.colors.foreground }}>Custom Open Graph</h3>
                    <p className="text-xs" style={{ color: theme.colors.foregroundMuted }}>Control how your profile appears when shared</p>
                  </div>
                </div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <span className="text-sm" style={{ color: theme.colors.foregroundMuted }}>Custom</span>
                  <div
                    className="relative w-12 h-6 rounded-full transition-all cursor-pointer"
                    style={{ background: currentOG.useCustom ? '#3b82f6' : theme.colors.backgroundSecondary }}
                    onClick={() => setLocalOG({ ...currentOG, useCustom: !currentOG.useCustom })}
                  >
                    <div
                      className="absolute top-1 w-4 h-4 rounded-full bg-white transition-all"
            ...
            <div className="p-5 space-y-4">
              <div>
                <label className="text-sm font-medium mb-2 block" style={{ color: theme.colors.foreground }}>Title</label>
                <input
                  type="text"
                  placeholder="Custom title for social sharing"
                  value={currentOG.title || ''}
                  onChange={(e) => setLocalOG({ ...currentOG, title: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl text-sm"
                  style={{ background: theme.colors.backgroundSecondary, color: theme.colors.foreground, border: `1px solid ${theme.colors.border}` }}
                />
              </div>
              <div>
                <label className="text-sm font-medium mb-2 block" style={{ color: theme.colors.foreground }}>Description</label>
                <textarea
                  placeholder="Custom description for social sharing"
                  value={currentOG.description || ''}
                  onChange={(e) => setLocalOG({ ...currentOG, description: e.target.value })}
                  rows={3}
              ...
              <div>
                <label className="text-sm font-medium mb-2 block" style={{ color: theme.colors.foreground }}>Image URL</label>
                <input
                  type="url"
                  placeholder="https://example.com/og-image.jpg"
                  value={currentOG.image || ''}
                  onChange={(e) => setLocalOG({ ...currentOG, image: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl text-sm"
                  style={{ background: theme.colors.backgroundSecondary, color: theme.colors.foreground, border: `1px solid ${theme.colors.border}` }}
                />
              </div>
              ...
              {currentOG.image && (
                <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${theme.colors.border}` }}>
                  <div className="p-3" style={{ background: theme.colors.backgroundSecondary }}>
                    <p className="text-xs font-medium" style={{ color: theme.colors.foregroundMuted }}>Preview</p>
                  </div>
                  <img src={currentOG.image} alt="OG Preview" className="w-full h-40 object-cover" />
                </div>
              )}
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
              >
                {ogMutation.isPending ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
        ...
        {activeSection === 'publish' && (
          ...
          >
            ...
            >
              <div className="p-5 border-b" style={{ borderColor: theme.colors.border }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold" style={{ color: theme.colors.foreground }}>Publish Template</h3>
                    <p className="text-xs" style={{ color: theme.colors.foregroundMuted }}>Share your style with the community</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 space-y-4">
                <div>
                  <label className="text-sm font-medium mb-2 block" style={{ color: theme.colors.foreground }}>Template Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block" style={{ color: theme.colors.foreground }}>Category</label>
                  <div className="grid grid-cols-5 gap-2">
                    {TEMPLATE_CATEGORIES.map((cat) => (
                      <button
                        ...
                        style={{
                          background: publishCategory === cat ? theme.colors.primary : theme.colors.backgroundSecondary,
                          color: publishCategory === cat ? '#fff' : theme.colors.foregroundMuted,
                        }}
                      >
                        {cat}
                      </button>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <div
                    ...
                    style={{ background: publishPublic ? '#22c55e' : theme.colors.backgroundSecondary }}
                  ...
                  <div>
                    <p className="text-sm font-medium" style={{ color: theme.colors.foreground }}>Public Template</p>
                    <p className="text-xs" style={{ color: theme.colors.foregroundMuted }}>Allow others to use your style</p>
                  </div>
                </label>

                <motion.button
                  onClick={() => publishName && publishMutation.mutate({ 
                    name: publishName, 
            ...
            {myTemplates && myTemplates.length > 0 && (
              ...
              >
                <div className="p-5 border-b" style={{ borderColor: theme.colors.border }}>
                  <h3 className="font-bold" style={{ color: theme.colors.foreground }}>Your Published Templates</h3>
                </div>
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-medium text-sm truncate" style={{ color: theme.colors.foreground }}>{template.name}</p>
                          {template.isPublic ? (
                        ...
                        <p className="text-xs" style={{ color: theme.colors.foregroundMuted }}>
                          {template.uses || 0} uses • {template.likes || 0} likes • {template.category}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <Link
            ...
            >
              <p className="text-sm" style={{ color: theme.colors.foreground }}>
                <strong>Tip:</strong> Publishing a template saves your current background, layout, CSS, fonts, and animation settings. 
                Other users can apply your style to their profiles with one click!
              </p>
            </div>
          </motion.div>
        )}
