      ...
      <div className="overflow-hidden backdrop-blur-xl p-6 rounded-lg bg-linear-to-br from-primary/10 via-transparent to-accent/10 border border-border/20">
        ...
        <div className="grid grid-cols-1 gap-4 mb-6">
          <div className="p-4 rounded-lg text-center bg-background-secondary/30">
            ...
            <p className="text-xs text-foreground-muted">{t('dashboard.referrals.totalReferrals')}</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {copied === 'code' ? <Check size={20} className="text-green-400" /> : <Copy size={20} className="text-foreground-muted" />}
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-3 rounded-xl bg-background-secondary/50 hover:bg-background-secondary transition-colors duration-(--animation-speed)"
              >
                {copied === 'link' ? <Check size={20} className="text-green-400" /> : <Copy size={20} className="text-foreground-muted" />}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-lg overflow-hidden bg-card/50 border border-border backdrop-blur-xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={item.step} className="flex items-center gap-3 p-4 rounded-xl bg-background-secondary/30">
          ...
          {stats?.referredUsers?.length ? (
            <div className="space-y-3">
              {stats.referredUsers.map((ref) => (
                <div key={ref.id} className="flex items-center gap-3 p-3 rounded-xl bg-background-secondary/30">
            ...
            <div className="text-center py-8">
              <Users size={32} className="mx-auto mb-3 text-foreground-muted/30" />
