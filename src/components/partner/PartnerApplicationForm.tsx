...
export function PartnerApplicationForm({ onClose, onSuccess }: Props) {
  ...
  const submitMutation = useMutation({
    mutationFn: (data: Parameters<typeof submitApplication>[0]) =>
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.whyPartner.length < 50) {
      setFormError(
        'Please explain why you want to partner (at least 50 characters)',
      )
      return
    }
    ...
    submitMutation.mutate({
      data: {
        ...
        audienceSize: formData.audienceSize as
          | 'micro'
          | 'small'
          | 'medium'
          | 'large'
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {STREAMING_PLATFORMS.slice(0, 6).map((platform) => (
                  <button
                    ...
                    style={{
                      background:
                        formData.categoryData.platform === platform.id
                          ? `${platform.color}20`
            ...
            {formData.categoryData.platform && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateCategoryData('platformUrl', e.target.value)
                  }
                  placeholder={
                    STREAMING_PLATFORMS.find(
                      (p) => p.id === formData.categoryData.platform,
                    )?.urlPrefix + '...'
                  }
              ...
              <div className="flex flex-wrap gap-2">
                {FOLLOWER_RANGES.map((range) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      updateCategoryData('followerRange', range.id)
                    }
                    className="px-3 py-1.5 rounded-lg text-sm font-medium transition-all"
                    style={{
                      background:
                        formData.categoryData.followerRange === range.id
                          ? 'rgba(20, 184, 166, 0.2)'
                          : 'rgba(255, 255, 255, 0.03)',
                ...
                <div className="flex flex-wrap gap-2">
                  {CONTENT_TYPES.map((type) => (
                    <button
                      ...
                      style={{
                        background:
                          formData.categoryData.contentTypes?.includes(type.id)
                            ? 'rgba(139, 92, 246, 0.2)'
                            : 'rgba(255, 255, 255, 0.03)',
                        border: `1px solid ${formData.categoryData.contentTypes?.includes(type.id) ? '#8b5cf6' : 'rgba(255, 255, 255, 0.08)'}`,
                        color: formData.categoryData.contentTypes?.includes(
                          type.id,
                        )
                          ? '#8b5cf6'
                          : 'var(--foreground-muted)',
              ...
              <div className="flex flex-wrap gap-2">
                {VTUBER_MODEL_TYPES.map((type) => (
                  <button
                    ...
                    style={{
                      background:
                        formData.categoryData.modelType === type.id
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateCategoryData('mainGamerTag', e.target.value)
                  }
                ...
                <div className="flex flex-wrap gap-2">
                  {TOP_GAMES.slice(0, 30).map((game) => (
                    <button
                      ...
                      style={{
                        background: formData.categoryData.games?.includes(game)
                          ? 'rgba(0, 212, 170, 0.2)'
                          : 'rgba(255, 255, 255, 0.03)',
                        border: `1px solid ${formData.categoryData.games?.includes(game) ? '#00D4AA' : 'rgba(255, 255, 255, 0.08)'}`,
                        color: formData.categoryData.games?.includes(game)
                          ? '#00D4AA'
                          : 'var(--foreground-muted)',
                      }}
                    >
                ...
                {GAME_PLATFORMS.slice(0, 6).map((platform) => (
                  <div key={platform.id} className="flex items-center gap-2">
                    <input
                      ...
                      value={
                        (
                          formData.categoryData.gamePlatforms as Record<
                            string,
                            string
                          >
                        )?.[platform.id] || ''
                      ...
                      onChange={(e) => {
                        const current =
                          (formData.categoryData.gamePlatforms as Record<
                            ...
                            string
                          >) || {}
                        updateCategoryData('gamePlatforms', {
                          ...current,
              ...
              <div className="grid grid-cols-2 gap-2">
                {DEVELOPER_TYPES.map((type) => (
                  <button
                    ...
                    style={{
                      background:
                        formData.categoryData.developerType === type.id
              ...
              <div className="flex flex-wrap gap-2">
                {PROGRAMMING_LANGUAGES.slice(0, 15).map((lang) => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg text-sm font-medium transition-all"
                    style={{
                      background: formData.categoryData.languages?.includes(
                        lang,
                      )
                        ? 'rgba(59, 130, 246, 0.2)'
                        : 'rgba(255, 255, 255, 0.03)',
                      border: `1px solid ${formData.categoryData.languages?.includes(lang) ? '#3B82F6' : 'rgba(255, 255, 255, 0.08)'}`,
                      color: formData.categoryData.languages?.includes(lang)
                        ? '#3B82F6'
                        : 'var(--foreground-muted)',
                    }}
                  >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateCategoryData('portfolioUrl', e.target.value)
                  }
        ...
        return (
          <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {GAME_ENGINES.map((engine) => (
                  <button
                    ...
                    style={{
                      background:
                        formData.categoryData.gameEngine === engine
                          ? 'rgba(245, 158, 11, 0.2)'
                          : 'rgba(255, 255, 255, 0.03)',
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {['C#', 'C++', 'GDScript', 'Lua', 'JavaScript', 'Python'].map(
                  (lang) => (
                    <button
                      key={lang}
                      type="button"
              ...
              <div className="flex flex-wrap gap-2">
                {ARTIST_TYPES.map((type) => (
                  <button
                    ...
                    style={{
                      background:
                        formData.categoryData.artistType === type.id
                          ? 'rgba(236, 72, 153, 0.2)'
                          : 'rgba(255, 255, 255, 0.03)',
              ...
              <div className="flex flex-wrap gap-2">
                {MUSICIAN_TYPES.map((type) => (
                  <button
                    ...
                    style={{
                      background:
                        formData.categoryData.musicianType === type.id
                          ? 'rgba(139, 92, 246, 0.2)'
                          : 'rgba(255, 255, 255, 0.03)',
                ...
                {MUSIC_PLATFORMS.map((platform) => (
                  <div key={platform.id} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={
                        (
                          formData.categoryData.musicPlatforms as Record<
                            string,
                            string
                          >
                        )?.[platform.id] || ''
                      ...
                      onChange={(e) => {
                        const current =
                          (formData.categoryData.musicPlatforms as Record<
                            ...
                            string
                          >) || {}
                        updateCategoryData('musicPlatforms', {
                          ...current,
                          [platform.id]: e.target.value,
                        })
                      }}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {FEATURED_CATEGORIES.map((cat) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="absolute top-0 right-0 px-2 py-0.5 rounded-bl-lg text-[10px] font-bold"
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, whyPartner: e.target.value })
                  }
                ...
                <div className="flex flex-wrap gap-2">
                  {FOLLOWER_RANGES.map((range) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, audienceSize: range.id })
                      }
                      className="px-4 py-2 rounded-xl text-sm font-medium transition-all"
                      style={{
                        background:
                          formData.audienceSize === range.id
                            ? 'rgba(139, 92, 246, 0.2)'
                            : 'rgba(255, 255, 255, 0.03)',
