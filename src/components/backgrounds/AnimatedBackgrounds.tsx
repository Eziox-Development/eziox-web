...
export function AnimatedBackground({
  ...
  useEffect(() => {
    ...
    const drawParticles = () => {
      ...
      particles.forEach((p) => {
        ...
        ctx.fillStyle = `${primaryColor}${Math.floor(p.opacity * 255)
          .toString(16)
          .padStart(2, '0')}`
        ctx.fill()
    ...
    const drawAurora = () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        gradient.addColorStop(
          0,
          `${primaryColor}${Math.floor(0.1 * intensityValue * 255)
            .toString(16)
            .padStart(2, '0')}`,
        )
        gradient.addColorStop(
          0.5,
          `${secondaryColor}${Math.floor(0.2 * intensityValue * 255)
            .toString(16)
            .padStart(2, '0')}`,
        )
        gradient.addColorStop(
    ...
    const drawFireflies = () => {
      ...
      particles.forEach((p) => {
        ...
        gradient.addColorStop(
          0.5,
          `rgba(255, 200, 100, ${glow * p.opacity * 0.3})`,
        )
    ...
    const drawCyberGrid = () => {
      ...
      const scanLine =
        ((time % 5) / 5) * canvas.height * 0.5 + canvas.height * 0.5
      ctx.beginPath()
    ...
    const drawBokeh = () => {
      ...
      particles.forEach((p) => {
        ...
        gradient.addColorStop(
          0,
          `${primaryColor}${Math.floor(p.opacity * 0.3 * 255)
            .toString(16)
            .padStart(2, '0')}`,
        )
        gradient.addColorStop(
          0.7,
          `${secondaryColor}${Math.floor(p.opacity * 0.1 * 255)
            .toString(16)
            .padStart(2, '0')}`,
        )
    ...
    const drawGeometric = () => {
      ...
      particles.forEach((p, i) => {
        ...
        ctx.closePath()
        ctx.strokeStyle = `${primaryColor}${Math.floor(p.opacity * 255)
          .toString(16)
          .padStart(2, '0')}`
        ctx.lineWidth = 2
        ctx.stroke()
    ...
    const drawNoise = () => {
      ...
      for (let i = 0; i < imageData.data.length; i += 4) {
        ...
        const noise =
          Math.sin(x * 0.02 + time) * Math.cos(y * 0.02 + time) * 0.5 + 0.5
    ...
    const drawVortex = () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        ctx.fillStyle = `${primaryColor}${Math.floor(
          (1 - i / 50) * intensityValue * 255,
        )
          .toString(16)
          .padStart(2, '0')}`
        ctx.fill()
  ...
  if (['gradient-mesh', 'fluid', 'holographic', 'neon-city'].includes(preset)) {
    return (
      <div
        className={`${positionClass} overflow-hidden pointer-events-none`}
        style={{ zIndex: 0 }}
      >
        <motion.div
