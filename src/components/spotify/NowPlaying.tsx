...
export function NowPlaying({
  ...
  const { data, isLoading } = useQuery<NowPlayingData>({
    ...
    queryFn: () =>
      userId ? getUserNowPlaying({ data: { userId } }) : getNowPlaying(),
    ...
    if (data?.isPlaying && data.track) {
      ...
      progressInterval.current = setInterval(() => {
        setLocalProgress((prev) => {
          ...
          return next >= (data.track?.duration || 0)
            ? data.track?.progress || 0
            : next
        })
      }, 1000)
    } else {
      ...
      >
        <div className={compact ? 'p-4' : 'p-5'}>
          <div className="flex items-center gap-4">
            <div
              className={`${compact ? 'w-12 h-12' : 'w-16 h-16'} rounded-2xl animate-pulse`}
  ...
  if (!data?.track) {
    ...
    return (
      ...
      >
        <div
          className="absolute -top-20 -right-20 w-40 h-40 opacity-10 pointer-events-none blur-3xl"
          style={{ background: SPOTIFY_GREEN }}
        />
        <div className={compact ? 'p-4' : 'p-6'}>
          <div className="flex items-center gap-4">
            <motion.div
              className={`${compact ? 'w-14 h-14' : 'w-16 h-16'} rounded-2xl flex items-center justify-center`}
              style={{
                background: `${SPOTIFY_GREEN}15`,
                border: `1px solid ${SPOTIFY_GREEN}20`,
              }}
            >
              <Music2
                size={compact ? 24 : 28}
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`${compact ? 'text-sm' : 'text-base'} font-semibold mb-1`}
                style={{ color: theme.colors.foreground }}
              >
                {t('spotify.nowPlaying.notPlaying')}
              </p>
              <span
                className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium"
                style={{
                  background: `${theme.colors.foregroundMuted}15`,
                  color: theme.colors.foregroundMuted,
                }}
              >
                <span
                  className="w-1.5 h-1.5 rounded-full"
                  style={{ background: theme.colors.foregroundMuted }}
                />
  ...
  if (compact) {
    return (
      <motion.a
        ...
        className="block rounded-3xl overflow-hidden relative backdrop-blur-xl group"
        style={cardStyle}
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <div
          className="absolute inset-0 opacity-30 blur-2xl scale-150 pointer-events-none"
          style={{
            backgroundImage: `url(${data.track.albumArt})`,
        ...
        <div className="relative p-4">
          ...
          <div
            ...
            style={{ background: `${theme.colors.backgroundSecondary}80` }}
          >
            <motion.div
  ...
  return (
    <motion.div
      className="rounded-3xl overflow-hidden relative backdrop-blur-xl"
      style={{ ...cardStyle, boxShadow: '0 12px 40px rgba(0,0,0,0.15)' }}
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      <div
        className="absolute inset-0 opacity-25 blur-3xl scale-150 pointer-events-none"
        style={{
          backgroundImage: `url(${data.track.albumArt})`,
          backgroundSize: 'cover',
        }}
      ...
      <div className="relative p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            <div
              ...
              style={{ background: `${SPOTIFY_GREEN}20` }}
            >
            ...
            >
              {data.isPlaying
                ? t('spotify.nowPlaying.nowPlaying')
                : t('spotify.nowPlaying.paused')}
            </span>
            {data.isPlaying && (
              <motion.div
                className="w-2 h-2 rounded-full"
          ...
          <motion.a
            ...
            style={{
              background: isHovered ? `${SPOTIFY_GREEN}20` : 'transparent',
              color: theme.colors.foregroundMuted,
            }}
        ...
        <div className="flex gap-5">
          ...
          >
            ...
            {data.isPlaying && !isHovered && (
              <div className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-black/60 rounded-lg px-2 py-1">
                <SoundBars />
              </div>
            )}
          </motion.div>
