    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        if (!permissions.profile?.read) {
          await logApiRequest(validation.apiKey.id, '/api/v1/me', 'GET', 403, Date.now() - startTime)
          return new Response(
        ...
        try {
          ...
          if (!result) {
            await logApiRequest(validation.apiKey.id, '/api/v1/me', 'GET', 404, Date.now() - startTime)
            return new Response(
          ...
          await logApiRequest(validation.apiKey.id, '/api/v1/me', 'GET', 200, Date.now() - startTime)
          ...
          await logApiRequest(validation.apiKey.id, '/api/v1/me', 'GET', 500, Date.now() - startTime, undefined, undefined, String(error))
          return new Response(
      ...
      PATCH: async ({ request }) => {
        ...
        if (!permissions.profile?.write) {
          await logApiRequest(validation.apiKey.id, '/api/v1/me', 'PATCH', 403, Date.now() - startTime)
          return new Response(
        ...
        try {
          ...
          const allowedFields = ['name', 'bio', 'location', 'website', 'pronouns']
          ...
          if (Object.keys(updates).length === 0) {
            await logApiRequest(validation.apiKey.id, '/api/v1/me', 'PATCH', 400, Date.now() - startTime)
            return new Response(
          ...
          await logApiRequest(validation.apiKey.id, '/api/v1/me', 'PATCH', 200, Date.now() - startTime)
          ...
          await logApiRequest(validation.apiKey.id, '/api/v1/me', 'PATCH', 500, Date.now() - startTime, undefined, undefined, String(error))
          return new Response(
            JSON.stringify({ error: 'Internal server error', code: 'INTERNAL_ERROR' }),
            { status: 500, headers: { 'Content-Type': 'application/json' } }
          )
