...
const validTabs: TabType[] = ['profile', 'links', 'referrals', 'badges', 'subscription', 'customization', 'creator', 'settings', 'privacy']

export const Route = createFileRoute('/_protected/profile')({
  validateSearch: (search: Record<string, unknown>): { tab?: TabType } => {
    const tab = search.tab as string | undefined
    return {
      tab: tab && validTabs.includes(tab as TabType) ? (tab as TabType) : undefined,
    }
  },
  head: () => ({
    meta: [
...
function ProfilePage() {
  ...
  return <ProfileDashboard currentUser={currentUser as Parameters<typeof ProfileDashboard>[0]['currentUser']} initialTab={tab} />
}
