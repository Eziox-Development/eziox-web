...
function BioPage() {
  ...
  const { data: followStatus } = useQuery({
    ...
    queryFn: () =>
      checkFollowingFn({ data: { targetUserId: profile!.user.id } }),
  ...
  const getPlatformFromUrl = (url: string) => {
    ...
    if (u.includes('discord'))
      return { name: 'Discord', color: '#5865F2', icon: SiDiscord }
    if (u.includes('github'))
      return { name: 'GitHub', color: '#fff', icon: SiGithub }
    if (u.includes('twitter') || u.includes('x.com'))
      return { name: 'X', color: '#fff', icon: SiX }
    if (u.includes('youtube') || u.includes('youtu.be'))
      return { name: 'YouTube', color: '#FF0000', icon: SiYoutube }
    if (u.includes('twitch'))
      return { name: 'Twitch', color: '#9146FF', icon: SiTwitch }
    if (u.includes('instagram'))
      return { name: 'Instagram', color: '#E4405F', icon: SiInstagram }
    if (u.includes('tiktok'))
      return { name: 'TikTok', color: '#fff', icon: SiTiktok }
    if (u.includes('spotify'))
      return { name: 'Spotify', color: '#1DB954', icon: SiSpotify }
    if (u.includes('soundcloud'))
      return { name: 'SoundCloud', color: '#FF5500', icon: SiSoundcloud }
    if (u.includes('linkedin'))
      return { name: 'LinkedIn', color: '#0A66C2', icon: SiLinkedin }
    return null
  }
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#0a0a0f]">
        ...
        >
          ...
          <h1 className="text-4xl font-bold text-white mb-4">
            {t('bioPage.notFound.title', { username })}
          </h1>
          <p className="text-lg text-white/50 mb-10">
            {t('bioPage.notFound.description')}
          </p>
          <Link
            to="/sign-up"
            className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl font-semibold text-white bg-gradient-to-r from-purple-600 to-cyan-600"
          >
            <Sparkles size={20} />
    ...
    <div className="min-h-screen relative" style={getBackgroundStyle()}>
      ...
      {(customBackground?.type === 'image' ||
        ...
        <div
          ...
          style={{
            background: `rgba(0,0,0,${1 - (customBackground.imageOpacity ?? 0.5)})`,
          }}
        />
      )}
        ...
        <div className="w-full max-w-xl" style={{ perspective: '1000px' }}>
          ...
          >
            ...
            >
              ...
              {/* Bio */}
              {profileData?.bio && (
                <p className="text-center text-white/70 mt-4 text-sm leading-relaxed">
                  {profileData.bio}
                </p>
              )}
              ...
              {isAuthenticated && !isSelf && (
                ...
                >
                  {isFollowLoading ? (
                    ...
                    <>
                      <UserMinus size={18} />
                      {t('bioPage.actions.following')}
                    </>
                  ) : (
                    <>
                      <UserPlus size={18} />
                      {t('bioPage.actions.follow')}
                    </>
                  )}
                </motion.button>
              )}
