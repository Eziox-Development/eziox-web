...
import type { CustomFont, AnimatedProfileSettings, OpenGraphSettings, LinkSchedule, ABTestVariant, EmbedSettings } from '../db/schema'
...
export const updateABTestFn = createServerFn({ method: 'POST' })
  .inputValidator(abTestSchema)
  .handler(async ({ data }) => {
    const user = await getAuthenticatedUser()
    const tier = await getUserTier(user.id)
    ...
    if (!isCreatorTier(tier)) {
      setResponseStatus(403)
      throw { message: 'A/B testing requires Creator tier or higher', status: 403, code: 'TIER_REQUIRED' }
    }
    ...
    await db
      .update(userLinks)
      .set({ 
        abTestEnabled: data.enabled,
        abTestVariants: (data.variants || []) as ABTestVariant[],
        updatedAt: new Date(),
      })
      .where(eq(userLinks.id, data.linkId))
...
export const updateLinkUTMFn = createServerFn({ method: 'POST' })
  .inputValidator(utmSchema)
  .handler(async ({ data }) => {
    const user = await getAuthenticatedUser()
    const tier = await getUserTier(user.id)
    ...
    if (!isCreatorTier(tier)) {
      setResponseStatus(403)
      throw { message: 'UTM parameters require Creator tier or higher', status: 403, code: 'TIER_REQUIRED' }
    }
...
const embedSettingsSchema = z.object({
  linkId: z.string().uuid(),
  settings: z.object({
    type: z.enum(['default', 'spotify', 'youtube', 'soundcloud']),
...
export const updateEmbedSettingsFn = createServerFn({ method: 'POST' })
  .inputValidator(embedSettingsSchema)
  .handler(async ({ data }) => {
    const user = await getAuthenticatedUser()
    const tier = await getUserTier(user.id)
    ...
    if (!isCreatorTier(tier)) {
      setResponseStatus(403)
      throw { message: 'Advanced embed controls require Creator tier or higher', status: 403, code: 'TIER_REQUIRED' }
    }
    ...
    await db
      .update(userLinks)
      .set({ 
        embedSettings: data.settings as EmbedSettings,
        updatedAt: new Date(),
      })
      .where(eq(userLinks.id, data.linkId))
