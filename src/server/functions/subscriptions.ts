...
async function updateUserBadgesForTier(
  userId: string,
  tier: TierType,
  ...
  if (tier === 'lifetime' && !currentBadges.includes('lifetime_subscriber')) {
    ...
    tier === 'creator' &&
    !currentBadges.includes('creator_subscriber')
  ) {
...
export const getCurrentSubscriptionFn = createServerFn({
  ...
  if (userData.stripeSubscriptionId) {
    const [sub] = await db
      ...
      .from(subscriptions)
      .where(
        eq(subscriptions.stripeSubscriptionId, userData.stripeSubscriptionId),
      )
      .limit(1)
  ...
  const normalizedTier =
    userData.tier === 'standard' || !userData.tier ? 'free' : userData.tier
...
export const createBillingPortalSessionFn = createServerFn({
  method: 'POST',
}).handler(async () => {
  if (!stripe) {
    throw { message: 'Stripe is not configured', status: 500 }
...
export const resumeSubscriptionFn = createServerFn({ method: 'POST' }).handler(
  async () => {
    if (!stripe) {
      throw { message: 'Stripe is not configured', status: 500 }
    }
...
async function handleSubscriptionCreated(
  ...
  const trialStartTs = (
    stripeSubscription as unknown as { trial_start: number | null }
  ).trial_start
  const trialEndTs = (
    stripeSubscription as unknown as { trial_end: number | null }
  ).trial_end
...
async function handleOneTimePayment(
  ...
  await db.insert(subscriptions).values({
    ...
    currentPeriodEnd:
      tierExpiresAt || new Date(Date.now() + 100 * 365 * 24 * 60 * 60 * 1000),
...
async function handleSubscriptionUpdated(subscription: {
  ...
  if (subscription.metadata?.userId) {
    ...
    await updateUserBadgesForTier(
      subscription.metadata.userId,
      effectiveTier as TierType,
    )
...
export const adminSetUserTierFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    await createSubscriptionNotification(
      data.userId,
      isUpgrade ? 'tier_granted' : 'downgraded',
      {
        tier: newTier,
        previousTier,
        expiresAt: tierExpiresAt,
  ...
  .inputValidator(
    z
      .object({
        ...
        tierFilter: z
          .enum(['all', 'free', 'pro', 'creator', 'lifetime'])
          .default('all'),
      })
      .optional(),
  )
  .handler(async ({ data }) => {
    ...
    const filteredResults =
      tierFilter === 'all'
        ...
        : results.filter((u) => {
            const normalizedTier =
              u.tier === 'standard' || !u.tier ? 'free' : u.tier
...
export const getCustomerBalanceFn = createServerFn({ method: 'GET' }).handler(
  async () => {
    if (!stripe) {
